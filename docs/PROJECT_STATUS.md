...
- הסשן האפקטיבי של האסטרטגיה (קבוע ב-ET): כניסות 09:00–11:15, flat ב-15:00.
  המודל שומר `entry_window` בשעון הסשן (08:45–11:30 ET) ומפרסם
  ‏`entry_window.txt` מתורגם לשעון הצ'ארט של העונה (קיץ: 07:45–10:30) —
  ה-Watch מפרסם מחדש בכל הפעלה (אחרי מעבר DST: להפעיל מחדש את ה-Watch).
- ‏override ידני: `entry_window_manual.txt` (למשל `9:30-16:00`, בשעון ET
  כמו ה-Scheduler של BlackBird) גובר על החלון האוטומטי.
- מחוץ לחלון: אין תוויות (bar_scores מסונן), באנר אפור, ‏MLPass=0.
- ‏walk-forward: ‏0.5422, סף 55 → ‏60% win על 60/192 (baseline ‏53.1%).
- **פער פתוח מול ה-Scheduler**: הבקטסט נכנס 09:00–11:15 ET ויוצא עד 15:00,
  אבל ה-Scheduler של BlackBird מציג 9:30–16:00 ET — כנראה ל-template
  ‏(PT_x) יש מגבלות זמן פנימיות. לבדוק ב-Edit BlackBird וליישר, ואז
  בקטסט חדש + export — הכל מתעדכן אוטומטית.
