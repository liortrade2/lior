...
- ‏`TimeOfDay_Min` נוסף ל-features (נגזר מ-DateTime; בר חי = שעון) —
  ‏walk-forward עלה ל-**0.5533**, סף 55 → ‏60% win על 60/192.
- כל עסקאות הבקטסט נכנסו 08:00–11:15 ⇒ המודל שומר `entry_window`
  ‏(465–690 דק'; קובץ `entry_window.txt` בתיקיית המכשיר). מחוץ לחלון:
  אין תוויות (bar_scores מסונן), הבאנר אפור ("no prediction"), ‏MLPass=0.
