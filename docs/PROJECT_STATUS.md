...
- ‏`entry_window.txt` מתפרסם **בדקות שעון-סשן** (ET אמיתי); האינדיקטורים
  ממירים כל בר בעצמם (`TOAIExporter.SessionMinutes`: ‏stamp ⇐ fixed UTC-5
  ⇐ Eastern) — נכון לכל בר היסטורי מכל עונת DST, בלי תלות ב-restart.
- ‏override ידני: `entry_window_manual.txt` (למשל `9:30-16:00`, בשעון ET
  כמו ה-Scheduler של BlackBird) גובר על החלון האוטומטי.
...
### המודל הנוכחי (2026-06-12 ‏10:00) — בקטסט שנתיים, הגדרות עדכניות
‏export ‏09:41: ‏ES SEP26, ינו 2024–ינו 2026, **239 עסקאות**, כניסות
‏10:00–13:15 ET (ליאור שינה את שעות האסטרטגיה — פרופיל חדש לעומת
‏09:00–11:15 הישן). חלון: 09:45–13:30 ET.
- ‏Walk-forward: **0.5406 ✔** (folds: ‏0.645, 0.475, 0.516, 0.526)
- טבלת ספים (WF): **70 → ‏58.2% win על 55/192** — השורה הטובה;
  ‏**55 → ‏50.8% — מתחת ל-baseline!** ⚠️ הסף החי כרגע 55 — מומלץ
  להעלות ל-70 בפאנל.
...
## תצורה חיה (C:\LIOR_ML)
- threshold.txt = **55**
- מכשיר פעיל: ‏ES (החוזה התגלגל ל-SEP26 — אותה תיקיית `ES\`), ‏15-min
- תיקיות נוספות: ‏MES, ‏NQ (טרם אומנו — אין model.pkl)
- שעון: המכונה והצ'ארט ב-Eastern; ה-timestamps מתנהגים כ-fixed UTC-5
  (היפוך DST מאומת בנתונים) — לכן כל חישובי הזמן עוברים דרך שעון-הסשן
