...
**מצב אסטרטגיות MES (2026-06-19):** ‏Two EMA+PSAR 15-דקות **הוסר** (ליאור: "לא יעיל"
חי). הפעיל כרגע: **Bollinger/RSI Mean Reversion 2-דקות** (וריאנטים 1a/1B). ⚠️ **אבל
ל-ML אין edge על האסטרטגיה הזו** — ראה ההנחיה לצ'אט הבא למטה. ‏BBTMP הישן הוצא משימוש.
...
## הצעד הראשון בצ'אט הבא (עדכון 2026-06-19)

### 🔑 הממצא הכי חשוב — ל-ML אין edge על האסטרטגיה הפעילה
הבדיקה המקיפה (15/15 עברו) + דיאגנוסטיקה על המודל החי (Bollinger/RSI 2-דקות, וריאנט 1B) הראתה:
- **Out-of-fold AUC = 0.472** (מתחת ל-0.5 = גרוע מאקראי בדירוג).
- win-rate חצי-עליון 48.3% מול חצי-תחתון 55.8% — **הציון הגבוה מנצח פחות**.
- ה-PMV ה-in-sample (0.61) היה **overfitting**; ה-walk-forward חשף שאין ניבוי אמיתי.
- **המסקנה:** על אסטרטגיית mean-reversion הזו, 11 הפיצ'רים לא מפרידים כניסות טובות מגרועות.
  זה **לא באג** — זו המערכת עושה את עבודתה (חושפת חוסר-edge במקום לסמוך על מספר מנופח).
- **השוואה:** ל-Two EMA+PSAR היה AUC≈0.62 (שם ה-ML *כן* הפריד), אבל ליאור הסירו ("לא יעיל" חי).

**המבחן ל-edge אמיתי (3 תנאים יחד):** ‏AUC/WF **>0.55-0.60** + win% עולה מונוטונית עם הציון +
עקומת ALLOW מעל "trading everything". כשאין שלושתם — ה-ML לא מרוויח כסף על האסטרטגיה ההיא.

### הצעד הבא — לבחור כיוון:
1. **לחפש אסטרטגיה שה-ML מפריד בה** (AUC>0.55) — זה הכיוון הנכון. לאמן אסטרטגיות/TF שונים
   (יש כפתור **⚙ Train export** + בורר TF פר-מכשיר), לבדוק ב-📊 Scorecard אם יש הפרדה אמיתית.
2. **או** לשפר הפרדה על הקיימת: עוד עסקאות, פיצ'רים ל-mean-reversion (מרחק מבולינגר, קיצוניות RSI),
   או וריאנט expectancy. (1a יש לו WF 0.56 > 1B 0.50 — אולי להפעיל אותו / ⚖ Compare.)
3. **או** לקבל שהאסטרטגיה הזו רווחית raw ולא צריך ML.

### החלטות פתוחות (תלויות-נתונים):
- **תיק חי** — צריך 2-3 אסטרטגיות מאומתות (WF>0.55, AUC>0.55) בקורלציה נמוכה. כרגע **0** עם edge מאומת.
