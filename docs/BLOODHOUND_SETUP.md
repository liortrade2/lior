...
## מעבר Timeframe (למשל 15-min → 1-min) — אוטומטי

ה-features ב-bar_data.csv תלויים ב-timeframe (ATR20 על ברים של דקה ≠ ATR20
על ברים של 15 דקות), ולכן אסור לערבב. **TOAIExporter מטפל בזה לבד:** הוא
זוכר (ב-bar_data_tf.txt) על איזה TF נבנה הקובץ, וכשפותחים צ'ארט על TF אחר —

1. ה-bar_data.csv הישן מאורכב אוטומטית (`bar_data_Minute-15_<תאריך>.csv`)
2. bar_scores.csv הישן נמחק (ציוני TF ישן חסרי משמעות) — ה-Watch בונה חדש
3. bar_data.csv חדש ונקי נבנה מהצ'ארט הנטען

כל מה שנשאר למשתמש: לפתוח צ'ארט על ה-TF החדש עם מספיק ימים לכסות את
תקופת הבקטסט, להריץ בקטסט על אותו TF ולשמור export — האימון יקרה אוטומטית.
